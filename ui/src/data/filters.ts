 ...
 */
import { uniqueAgentById } from "./agents";
...
export function applyFilters(env: Envelope, filters: FindingFilters, framework: FrameworkId): FindingRef[] {
  const source = filters.status === "suppressed" ? allFindings(env).filter((r) => r.finding.suppressed) : filters.status ? activeFindings(env) : allFindings(env);
  const q = filters.q.trim().toLowerCase();
  // The agent filter always means the unique agent, whether the link names it or one of its scanned records.
  const wanted = filters.agent ? uniqueAgentById(env, filters.agent) : null;
  return source.filter(({ finding, agents, uniqueAgents }) => {
    ...
    if (wanted && !uniqueAgents.includes(wanted)) return false;
    if (filters.agent && !wanted) return false;
    ...
    if (q) {
      const hay = `${finding.rule_id} ${finding.title} ${finding.path} ${finding.snippet} ${finding.remediation} ${finding.crosswalk?.so_what ?? ""} ${finding.canonical_name ?? ""} ${agents.map((a) => a.display_name || a.name).join(" ")} ${uniqueAgents.map((u) => u.name).join(" ")}`.toLowerCase();
