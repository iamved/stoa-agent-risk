...
export async function reply(req: Request) {
  ...
  const tools = { create_ticket: { description: 'file a Zendesk ticket' },
                  hand_off: { description: 'escalate to a human' } };
  const out = await generateText({ model: openai('gpt-4o'), system: prompt, tools });
